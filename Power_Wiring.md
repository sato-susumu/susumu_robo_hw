...
| 系統 | 電源 | 給電対象 |
|------|------|----------|
| 20V 系 | [DeWalt 20V バッテリー](Battery.md) | [ODrive S1](ODrive_S1.md) × 2、[Livox MID-360](Livox_MID360.md)、5V 補機 ([USB-HUB-7U](USB-HUB-7U.md)) |
...
| 独立 | 内蔵バッテリー (USB 充電) | [AmiVoice WT01](AmiVoice_WT01.md) (BT マイク、本電源系統には接続しない) |
...
flowchart LR
    ...
    MOTL["BotWheel 左<br/>171mm ハブモータ<br/>(エンコーダ/ホール/サーミスタ内蔵)"]
    MOTR["BotWheel 右<br/>171mm ハブモータ<br/>(エンコーダ/ホール/サーミスタ内蔵)"]
    ...
    %% ===== 独立機器 (本電源系統に接続せず) =====
    WT01["AmiVoice WT01<br/>(BTマイク・独立電源/独立充電)"]
    ...
    class MOTL,MOTR,D435,MID,WT01 sensor
```
...
- **5V 補機**: [Buck DCDC (10-32V → 5V 6A)](DCDC_Converter.md) を介して [USB-HUB-7U](USB-HUB-7U.md) を給電。USB Hub 経由で低速 USB デバイスを集約 (現時点で常時接続する USB デバイスは未確定)。
- **独立機器**: [AmiVoice WT01](AmiVoice_WT01.md) は内蔵バッテリー駆動・USB 充電の独立機器で、本電源系統には接続しない。
...
| PC ↔ USB-HUB-7U | USB 2.0 | 低速 USB デバイスを集約 |
| PC ↔ Livox MID-360 | Ethernet (100BASE-TX) | UM790 Pro 内蔵 2.5GbE ポートに直結 |
